...
public:
	...
	static clsUser _ConvertLinetoUserObject(string LineData, string Sepertor = "#//#")
	{
		vector<string> vWord = clsString::Split(LineData, Sepertor);
		return clsUser(enMode::UpdateMode, vWord[0], vWord[1], vWord[2], vWord[3], vWord[4], vWord[5], stod(vWord[6]));

	}
	static stLoginRegisterRecord _ConvertLinetoLoginRegisterRcord(string LineData, string Sepertor = "#//#")
	{
	...
	static vector<clsUser> _LoadUserDataFromFaile()
	{
		...
		if (MyFile.is_open())
		{
			...
			{
				clsUser UserInfo = _ConvertLinetoUserObject(line);
	...
	static vector<stLoginRegisterRecord> GetLoginRegisterList()
	{
		...
		if (MyFile.is_open())
		{
			while (getline(MyFile, line))
			{
				stLoginRegisterRecord LoginRegisterRecord = _ConvertLinetoLoginRegisterRcord(line);
