...
private:
	...
	static int ReadAmount(clsBankClient Client)
	{
		int TransferAmount = 0;
		cout << "Enter Transfer Amount ? ";
		TransferAmount = clsInputValidate::ReadIntNumberBetween(1, Client.AccountBalance, "Amount Exceeds the available Balance ,Enter another Amount ? ");
...
public:
	static void ShowTransferScreen()
	{
		...
		int TransferAmount = ReadAmount(SourceClient);
		...
		if (toupper(Answer) == 'Y')
		{
			if (SourceClient.Transfer(TransferAmount, DestinationClinet))
			{
